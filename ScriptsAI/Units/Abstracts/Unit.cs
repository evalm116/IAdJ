...
public abstract class Unit : MonoBehaviour
{
    ...
    void Update()
    {
        // TODO: esto esta aqu� hasta que se haga el arbol de comportamiento de las unidades, luego se borra
        autoAttack();
        ...
        for (int i = 0; i < grid.columnas; i++)
        {
            for (int j = 0; j < grid.filas; j++)
            {
                ...
                if (cell != null)
                {
                    ...
                    // Si est� dentro del radio de b�squeda y es la m�s cercana
    ...
    private void Die()
    {
        ...
        // TODO: Esto lo debe hacer el �rbol de comportamiento
        FindObjectsByType<GameManager>(FindObjectsSortMode.None)[0].RegisterDeadUnit(this);
    }


    /// <summary>
    /// Puede respawnear si est� muerto y pas� el tiempo suficiente
    /// </summary>
    ...
    public void Respawn(Vector3 respawnPoint)
    {
        if (!_isDead)
        {
            Debug.LogWarning($"{this.type} est� vivo, no puede respawnear.");
            return;
        }
